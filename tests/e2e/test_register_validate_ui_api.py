...
def test_register_via_ui_and_validate_customer_via_api(page, settings):
    ...
    with log_step(logger, f"Register user in UI (username={user['username']})"):
        register_page.open(timeout_ms=settings.timeout_ms)
        register_page.register(user)
        register_page.assert_registration_success()

    try:
        with log_step(logger, "Validate user login through API"):
    ...
    with log_step(logger, f"Fetch customer and validate profile (customer_id={customer_id})"):
